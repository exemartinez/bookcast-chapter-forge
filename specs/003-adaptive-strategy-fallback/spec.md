...
3. **Given** the primary cascade `regex -> layout -> llm` is exhausted without an accepted result, **When** the `adaptive` strategy runs, **Then** it tries a secondary pool containing `index`, `heuristic`, and `semantic` in randomized order and stops at the first accepted result.
4. **Given** the first attempted strategy produces output that is accepted as sensible, **When** the `adaptive` strategy runs, **Then** it does not invoke later fallback steps.
...
4. **Given** the primary cascade is exhausted after deterministic rejections or runtime failures, **When** the wrapper enters the secondary pool, **Then** it continues trying those strategies one by one until one passes or all fail.
...
- `regex -> layout -> llm` all fail or are rejected, but `index`, `heuristic`, or `semantic` can still produce an acceptable result.
...
- **FR-003**: The `adaptive` strategy MUST try existing strategies in this default deterministic primary order unless later planning explicitly changes it: `regex -> layout -> llm`.
- **FR-003a**: If the primary cascade is exhausted without an accepted result, the `adaptive` strategy MUST try a secondary fallback pool containing `index`, `heuristic`, and `semantic` in randomized order.
...
- **FR-012a**: The `adaptive` wrapper MUST stop at the first strategy from either the primary cascade or the secondary fallback pool that returns an accepted result.
...
- **SC-002a**: On a PDF where `regex -> layout -> llm` runs dry but one of `index`, `heuristic`, or `semantic` succeeds, `adaptive` continues into the secondary pool and returns the first accepted result without manual reruns.
...
- **SC-005**: Adaptive-strategy tests cover dependency/runtime failures, rejected-output continuation, accepted-winner selection, secondary-pool continuation after the primary cascade runs dry, and LLM sensibility checks for duplicate filenames and low file counts.
...
- The default fallback behavior for feature 003 is a primary cascade of `regex -> layout -> llm`, followed by a randomized secondary pool of `index`, `heuristic`, and `semantic` only if the primary cascade runs dry.
